  ...
  return (
    <Layout title="Going Dutch">
      <section className="mb-8 flex items-end justify-between gap-4">
        <div><p className="section-label mb-2">{t('sharedSpending')}</p><h2 className="text-3xl font-semibold tracking-tight text-foreground">{t('yourGroups')}</h2></div>
        <div className="flex shrink-0 gap-2"><Button variant="outline" onClick={() => setJoinOpen(true)} className="gap-2"><Wifi className="h-4 w-4" /><span className="hidden sm:inline">{t('joinSync')}</span></Button><Button onClick={() => navigate('/create-group')} className="app-button-primary gap-2"><Plus className="h-4 w-4" /><span className="hidden sm:inline">{t('newGroup')}</span></Button></div>
      </section>
      ...
      {groups.length > 0 && <section className="app-surface-strong mb-8 grid gap-5 p-5 sm:grid-cols-[1fr_auto] sm:p-6">
        <div><p className="section-label mb-2">{t('totalSharedSpending')}</p><p className="amount text-3xl font-semibold text-foreground sm:text-4xl">{formatVnd(totalSpent)}</p><p className="mt-2 text-sm text-muted-foreground">{groups.length} {t('groups')} · {totalExpenses} {t('expenses')}</p></div>
        <div className="grid grid-cols-2 gap-3 sm:min-w-52"><div className="app-surface p-3"><Users className="mb-4 h-4 w-4 balance-positive" /><p className="text-xl font-semibold">{totalMembers}</p><p className="text-xs text-muted-foreground">{t('members')}</p></div><div className="app-surface p-3"><ReceiptText className="mb-4 h-4 w-4 balance-positive" /><p className="text-xl font-semibold">{totalExpenses}</p><p className="text-xs text-muted-foreground">{t('expenses')}</p></div></div>
      </section>}
      ...
      {groups.length > 0 ? (
        ...
        <div className="app-surface mt-8 flex flex-col items-center px-6 py-16 text-center">
          ...
          <h3 className="text-xl font-semibold text-foreground mb-2">{t('noGroupsYet')}</h3>
          ...
          >
            <Plus className="w-4 h-4" /> {t('createFirstGroup')}
          </Button>
        </div>
      )}
